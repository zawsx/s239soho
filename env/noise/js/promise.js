;(function(){/** * Require the given path. * * @param {String} path * @return {Object} exports * @api public */
function require(path, parent, orig) {var resolved = require.resolve(path);
  // lookup failed
  if (null == resolved) { orig = orig || path;  parent = parent || 'root';
  ...
  if (!module.exports) {module.exports = {}; module.client = module.component = true;
    module.call(this, module.exports, require.relative(resolved), module);  }
  return module.exports;}
/** Registered modules. */require.modules = {};
/** Registered aliases. */require.aliases = {};
/** Resolve `path`. Lookup:  - PATH/index.js - PATH.js  - PATH
 * @param {String} path @return {String} path or null @api private */
require.resolve = function(path) {if (path.charAt(0) === '/') path = path.slice(1);
  ...
  for (var i = 0; i < paths.length; i++) {var path = paths[i]; if (require.modules.hasOwnProperty(path)) return path;
    if (require.aliases.hasOwnProperty(path)) return require.aliases[path]; }};
/** Normalize `path` relative to the current path. @param {String} curr @param {String} path @return {String} @api private */
require.normalize = function(curr, path) {var segs = []; if ('.' != path.charAt(0)) return path;
  curr = curr.split('/');  path = path.split('/');
  for (var i = 0; i < path.length; ++i) {if ('..' == path[i]) { curr.pop();} 
